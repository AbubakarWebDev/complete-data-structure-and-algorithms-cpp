...
2) Priority Queue: Priority queues can be efficiently implemented using Binary Heap because it supports insert(), delete() and extractmax(), increaseKey() operations in O(logn) time. Binomoial Heap and Fibonacci Heap are variations of Binary Heap. These variations perform union also efficiently.
...
1) getMax(): It returns the root element of Min Heap. Time Complexity of this operation is O(1).

2) extractMax(): Removes the maximum element from MinHeap. Time Complexity of this Operation is O(Logn) as this operation needs to maintain the heap property (by calling heapify()) after removing root.

3) increaseKey(): Increases value of key. The time complexity of this operation is O(Logn). If the increases key value of a node is greater than the parent of the node, then we don’t need to do anything. Otherwise, we need to traverse up to fix the violated heap property.
...
// new_val is greater than array[i].
