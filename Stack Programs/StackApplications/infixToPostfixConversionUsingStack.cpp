/*******************************************************************
Steps to Manually Convert Infix to Postfix:
1) Apply appropriate brackets/paranthesis to expression by the operator precedence and associativity rules.
2) Start by Converting Inner most sub expression to postfix and name it with temporary placeholder.
...
For Example: Convert the Following Infix Expression to Postfix: a + b * c - d / e
...
Step 2: Start by Converting Inner most sub expression to postfix and name it with temporary placeholder:
[a + X1 - X2]       // X1 = 'bc*'  X2 = 'de/'
...
[X3X2-]
So According to the step 5 we solved the above expression completely from left to right.
...
Steps to Convert Infix to Postfix using stack data structures:
1) Scan Expression from Left to Right.
2) Print OPERANDs as the arrive.
3) IF incoming SYMBOL is ‘(‘ then PUSH it onto Stack.
4) IF incoming SYMBOL is ‘)’ then we POP the stack and print all the stack values till stacks gets empty OR  ‘(‘ is found. If ‘(‘ is found then POP it but never print it.
5) If OPERATOR arrives & Stack is empty, push this operator onto the stack.
6) But IF Stack is Not Empty Check incoming OPERATOR has HIGHER precedence than the TOP of the Stack, push incoming OPERATOR on stack. IF the TOP of stack is ‘(‘ PUSH incoming OPERATOR on Stack.
7) IF incoming OPERATOR has LOWER precedence than the TOP of the Stack, then POP and print the TOP. Then compares the incoming OPERATOR With the NEW TOP of stack. IF incoming OPERATOR has HIGHER precedence follow step '6' OR if LOWER precedence than follow step '7' OR if EQUAL precedence than follow step '8'.
8) IF incoming OPERATOR has EQUAL precedence with TOP of Stack, use the below ASSOCIATIVITY Rules.
9) For ASSOCIATIVITY of LEFT to RIGHT: POP and print the TOP of stack, then push the incoming OPERATOR
10) For ASSOCIATIVITY of RIGHT to LEFT: PUSH incoming OPERATOR on stack.
11) At the end of Expression, POP & print all OPERATORS from the stack
----------------------------------------------------------------------------------------

PSEUDOCODE for Converting Infix to Postfix expressions using stack data structures:

FUNCTION  infixToPostfix(stack, infix)
    
    Declare Variable: String postfix

    FOR LOOP (i = 0; i < infix.length; i++)
        IF CHECK_OPERAND(infix[i])
            THEN postfix += infix[i]
        ELSEIF infix[i] == '('
...
int main()
{
    ...
    postfix_exp = infixToPostfix(stk, infix_exp);
